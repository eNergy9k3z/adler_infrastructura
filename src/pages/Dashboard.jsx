  ...
  function closeDetail() {
    if (leaveDetail())
      requestAnimationFrame(() => {
        const trigger =
          document.getElementById(`inbox-contact-${selectedTrigger.current}`) ||
  ...
  function changeFilter(value) {
    if (!exportState.busy && leaveDetail()) {
      ...
      if (foldersOpen)
        requestAnimationFrame(() =>
          document
            .getElementById("inbox-list-title")
  ...
  return (
    <section className={`inbox-page ${foldersOpen ? "folders-open" : ""}`}>
      <header className="inbox-appbar">
        ...
        >
          <button type="submit" aria-label="Buscar" disabled={exportState.busy}>
            <Search size={19} />
          </button>
        ...
        <span className="inbox-account" title={session.user.email}>
          <span>{session.user.email}</span>
          <span className="inbox-account-avatar" aria-hidden="true">
            <UserRound size={18} />
          </span>
        </span>
      </header>
      <nav className="inbox-app-rail" aria-label="Aplicaciones de Adler">
        <Link
          to="/dashboard"
          aria-current="page"
          aria-label="Consultas de la web"
          title="Consultas de la web"
        >
          <Mail size={22} />
        </Link>
        <Link
        ...
        <Link to="/" aria-label="Ver página web" title="Ver página web">
          <ExternalLink size={21} />
        </Link>
      </nav>
      <div className="inbox-commandbar">
        <button
          className="inbox-folder-toggle"
          aria-label={foldersOpen ? "Ocultar carpetas" : "Mostrar carpetas"}
          aria-expanded={foldersOpen}
          aria-controls="inbox-folders"
        ...
        <button
          ...
          title="Descargar todos los resultados filtrados"
          aria-describedby="inbox-export-hint"
        >
          <Download size={17} />
          {exportState.busy ? "Preparando…" : "Descargar Excel"}
        </button>
        ...
        >
          <RefreshCw size={17} className={loading ? "sending-spinner" : ""} />
          Actualizar
        </button>
        <Link
          className="inbox-toolbar-button inbox-client-link"
          to="/dashboard/clientes"
        >
      ...
      <aside
        className="inbox-folders"
        id="inbox-folders"
        aria-label="Carpetas de consultas"
      >
        <div className="inbox-mailbox">
          <ChevronDown size={14} />
          <span>Adler Infrastructura</span>
        </div>
        <nav aria-label="Filtrar consultas por estado">
          {[
            ["", "Bandeja de entrada", <Inbox size={18} />],
            ["pendiente", "Pendientes", <Clock3 size={18} />],
            ["seguimiento", "En seguimiento", <Flag size={18} />],
            ["resuelta", "Resueltas", <CircleCheck size={18} />],
        ...
        <div className="inbox-folder-links">
          ...
          <Link to="/dashboard/clientes/directorio">
            <UserRound size={18} />
      ...
      <div className="inbox-main">
        {exportState.message && (
          <div
            className={`inbox-export-feedback ${exportState.error ? "is-error" : ""}`}
        ...
        <div className={`inbox-workspace ${selected ? "has-detail" : ""}`}>
          ...
          >
            <header className="inbox-list-header">
              <h1 id="inbox-list-title" tabIndex={-1}>
                {folderName}
              </h1>
              <p role="status">
                {loading
            ...
            <div className="inbox-list-context">
              <span>
                {query ? `Resultados para «${query}»` : "Más recientes primero"}
              </span>
              <span id="inbox-export-hint" className="sr-only">
                Excel incluye todos los resultados filtrados.
              </span>
            </div>
            <div className="inbox-messages-scroll">
              {loading ? (
                <div className="inbox-empty" role="status">
                  <RefreshCw size={23} className="sending-spinner" />
                  Cargando consultas…
                </div>
              ) : error ? (
                ...
                <div className="inbox-empty">
                  ...
                  <p>
                    {query || filter
                      ? "Prueba otro término o abre la bandeja de entrada."
                      : "Los mensajes del formulario aparecerán aquí."}
                  </p>
                </div>
              ) : (
                    ...
                    return (
                      <li key={item.id}>
                        <button
                          ...
                          className={`inbox-message-row ${selected?.id === item.id ? "is-selected" : ""}`}
                          onClick={() => openContact(item)}
                          aria-label={`Abrir consulta de ${item.name || "Contacto sin nombre"}`}
                        ...
                        >
                          ...
                          <span className="inbox-message-summary">
                            <span className="inbox-message-top">
                              <span
                                className="inbox-contact-name"
                                title={item.name || "Contacto sin nombre"}
                              >
                            ...
                            <span className="inbox-topic">
                              {summary.service}
                            </span>
                            <span className="inbox-cell-preview">
                              {summary.message || "Sin texto adicional"}
                            </span>
                            <span className="inbox-message-meta">
                              <span
                                className={`inbox-state-label status-${item.status}`}
                              >
                                {labels[item.status]}
                              </span>
                              <span className="inbox-message-company">
            ...
            {!loading && !error && result.total > 0 && (
              ...
              >
                <span>
                  {page * 50 + 1}–{Math.min((page + 1) * 50, result.total)} de{" "}
                  {result.total}
                </span>
                <div>
                  <button
          ...
          {selected ? (
            ...
            <div className="inbox-reading-placeholder">
              <span>
                <MailOpen size={46} strokeWidth={1.1} />
              </span>
              <h2>Selecciona una consulta</h2>
              <p>Lee el mensaje y gestiona su seguimiento aquí.</p>
            </div>
          )}
        </div>
      </div>
...
function consultationSummary(message = "") {
  ...
  return {
    service: match ? match[1] : "Consulta general",
  ...
  return (
    ...
    >
      <header className="inbox-detail-header">
        ...
        >
          <ArrowLeft size={18} className="inbox-mobile-back" />
          <span className="inbox-mobile-back">Bandeja</span>
          <X size={19} className="inbox-desktop-close" />
        </button>
      </header>
      <div className="inbox-detail-scroll">
        <h2 className="inbox-reading-subject">
          {consultationSummary(item.message).service}
        </h2>
        <div className="inbox-detail-identity">
          <span className="inbox-detail-avatar" aria-hidden="true">
            {initials(item.name)}
          </span>
          <div>
            <h3>{item.name || "Contacto sin nombre"}</h3>
            <p>{item.email || "Correo no registrado"}</p>
            <p>Para: Adler Infrastructura</p>
          </div>
          <time className="inbox-detail-date" dateTime={item.created_at}>
            {date(item.created_at)}
          </time>
        </div>
        <details className="inbox-contact-data">
          <summary>
            Datos del contacto
            <ChevronDown size={14} />
          </summary>
        ...
        <div className="inbox-reading-body">
          <p className="inbox-message">
            {consultationSummary(item.message).message ||
              "Sin texto adicional."}
          </p>
          {safeEmail && (
            <a
              ...
              title="Abre tu aplicación de correo"
            >
              <Reply size={17} />
          ...
          {safeEmail && (
            <p className="inbox-private-hint">
              Se abrirá tu aplicación de correo. La respuesta no se guarda en
              esta bandeja.
            </p>
          )}
        </div>
        <div className="inbox-followup-heading">
          <h3>Seguimiento de la consulta</h3>
          <span>
            <ShieldCheck size={14} />
            Solo Adler
          </span>
        ...
        {saveState === "conflict" && (
          <p role="alert" className="private-error">
            La consulta cambió o tu acceso ya no está disponible. Copia tus
            notas y actualiza la bandeja antes de guardar.
          </p>
        )}
      </div>
      <footer className="inbox-detail-footer">
        <span
          role="status"
          className={changed ? "inbox-unsaved" : "inbox-saved"}
        >
          {saveState === "saving"
            ? "Guardando cambios…"
            : changed
